...
# Sets the default device of tensors to the GPU if available. 
...
if __name__ == '__main__':
    ...
    with Timer('Total time'):
        ...
        W = layers.Input((512, 3072), train=True)
        W.randomize()
        b = layers.Input((512, 1), train=True)
        b.randomize()
        x = layers.Input((3072, batch_size), train=False)
        # x input set when network.forward() called 

        linear_1 = layers.Linear(x, W, b) # u
        relu = layers.ReLU(linear_1) # h

        M = layers.Input((10, 512), train=True)
        M.randomize()
    ...
    # Plot the training and testing accuracy and loss for each epoch
    ...
    plt.xlabel('Epochs')
    plt.ylabel('Accuracy')
    plt.title('CIFAR-10 Testing Accuracy')

    plt.plot(range(len(test_losses)), test_losses)
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.title('CIFAR-10 Testing Loss')
